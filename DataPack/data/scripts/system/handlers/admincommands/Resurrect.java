...
public class Resurrect extends AdminCommand {
	 ...
	 * with the instant flag, it may bug the player. Must wait for 2 or 3 second
	 * after death before using resurrect instant. (Prompt may be used
	 * immediately.)
	 */
	@Override
	public void executeCommand(Player admin, String[] params) {
		...
		if (!(target instanceof Player)) {
			PacketSendUtility.sendMessage(admin,
					"You can only resurrect other players.");
			return;
		}
		...
		if (!player.getLifeStats().isAlreadyDead()) {
			PacketSendUtility.sendMessage(admin,
					"That player is already alive.");
			return;
		}

		// Default action is to prompt for resurrect.
		if (params == null || params.length == 0
				|| ("prompt").startsWith(params[0])) {
		...
		PacketSendUtility
				.sendMessage(admin,
						"[Resurrect] Usage: target player and use //rez <instant|prompt>");
	}

...
public class Appearance extends AdminCommand {
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if (params[0].equals("reset")) {
			PlayerAppearance savedPlayerAppearance = player
					.getSavedPlayerAppearance();

			if (savedPlayerAppearance == null) {
				PacketSendUtility.sendMessage(admin,
						"The target has already the normal appearance.");
				return;
			}
			...
			PacketSendUtility.sendMessage(player,
					"An admin has resetted your appearance.");
		...
		if (player.getSavedPlayerAppearance() == null)
			player.setSavedPlayerAppearance((PlayerAppearance) playerAppearance
					.clone());
		...
		} else if (params[0].equals("head_size")) // Min: 0, Max: 100
		{
			...
			if (head < 0 || head > 100) {
				PacketSendUtility.sendMessage(admin,
						"Head Size: Min value : 0 - Max value : 100");
				return;
			}
		...
		} else if (params[0].equals("tattoo")) // Min: 1, Max: 13
		{
			...
			if (tattoo < 1 || tattoo > 13) {
				PacketSendUtility.sendMessage(admin,
						"Tattoo: Min value : 1 - Max value : 13");
				return;
			}
		...
		// Warn the player
		PacketSendUtility.sendMessage(player,
				"An admin has changed your appearance.");

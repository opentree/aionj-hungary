 ...
 * @author Sarynth
 * 
 *         Simple admin assistance command for adding kinah to self, named
 *         player or target player.
 * 
 *         Based on //add command. Kinah Item Id - 182400001 (Using
 *         ItemId.KINAH.value())
 * 
 */
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if (params.length == 1) {
			...
			try {
				...
				PacketSendUtility.sendMessage(admin,
						"Kinah value must be an integer.");
				return;
			}
		} else {
		...
		PacketSendUtility.sendMessage(receiver,
				"An admin gives you some kinah.");
	}

	public static void main(String[] args) {
		AdminCommandChatHandler.getInstance().registerAdminCommand(new Kinah());
	}
}

...
public class Dye extends AdminCommand {
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if (params.length == 0 || params.length > 2) {
			PacketSendUtility.sendMessage(admin,
					"syntax //dye <dye color|hex color|no>");
			return;
		}
		...
		else if (color.equalsIgnoreCase("true white")
				|| color.equalsIgnoreCase("white")) {
			color = "ffffff";
		} // 169200006, 169201006, 169220002, 169231002
		else if (color.equalsIgnoreCase("black")
				|| color.equalsIgnoreCase("true black")) {
			color = "000000";
		...
		try {
			...
			if (!color.equalsIgnoreCase("no")) {
				PacketSendUtility.sendMessage(admin, "[Dye] Can't understand: "
						+ color);
				return;
			}
		}

		for (Item targetItem : target.getEquipment()
			...
			PacketSendUtility
					.sendPacket(target, new SM_UPDATE_ITEM(targetItem));
		}
		PacketSendUtility.broadcastPacket(target,
				new SM_UPDATE_PLAYER_APPEARANCE(target.getObjectId(), target
						.getEquipment().getEquippedItemsWithoutStigma()), true);
		target.getEquipment().setPersistentState(
				PersistentState.UPDATE_REQUIRED);
		if (target.getObjectId() != admin.getObjectId()) {
			PacketSendUtility.sendMessage(target, "You have been dyed by "
					+ admin.getName() + "!");
		}
		PacketSendUtility.sendMessage(admin, "Dyed " + target.getName()
				+ " successfully!");
	}

	public static void main(String[] args) {
		AdminCommandChatHandler.getInstance().registerAdminCommand(new Dye());
	}
}

...
public class AddTitle extends AdminCommand {
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if ((titleId > 50) || (titleId < 1)) {
			PacketSendUtility.sendMessage(admin, "title id " + titleId
					+ " is invalid (must be between 1 and 50)");
			return;
		}

		Player target = null;
		if (params.length == 2) {
			target = World.getInstance()
					.findPlayer(Util.convertName(params[1]));
			if (target == null) {
				PacketSendUtility.sendMessage(admin, "player " + params[1]
						+ " was not found");
				return;
			}
		} else {
		...
		if (!target.getTitleList().addTitle(titleId)) {
			PacketSendUtility.sendMessage(admin, "you can't add title #"
					+ titleId + " to "
					+ (target.equals(admin) ? "yourself" : target.getName()));
		} else {
			if (target.equals(admin)) {
				PacketSendUtility.sendMessage(admin,
						"you added to yourself title #" + titleId);
			} else {
				PacketSendUtility.sendMessage(admin,
						"you added to " + target.getName() + " title #"
								+ titleId);
				PacketSendUtility.sendMessage(target, admin.getName()
						+ " gave you title #" + titleId);
			}
		}
	}

...
public class Siege extends AdminCommand
{
	...
	private void processList(Player admin, String[] params)
	{
		...
		for (FastMap.Entry<Integer, SiegeLocation> e = locations.head(), end = locations.tail(); (e = e.getNext()) != end;)
		{
			...
			msg += " - " + (sLoc instanceof Artifact ? "Artifact" : (sLoc instanceof Fortress ? "Fortress" : "Other")) + " " + sLoc.getLocationId() + " ("
					+ sLoc.getRace().toString() + ")" + (sLoc.getLegionId() == 0 ? "" : " " + sLoc.getLegionId()) + "\n";
	...
	{
		PacketSendUtility.sendMessage(admin, "[Help: Siege List Command]\n" + "  The siege list command outputs each siege location."
	...
	{
		PacketSendUtility.sendMessage(admin, "Syntax: //siege set <location id> <current state> [next state]\n"
	...
	{
		PacketSendUtility.sendMessage(admin, "Syntax: //siege capture <location id> <race> [legion id]\n"

...
public class Ban extends AdminCommand {
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if (params == null || params.length < 1) {
			PacketSendUtility
					.sendMessage(admin,
							"Syntax: //ban <player> [account|ip|full] [time in minutes]");
			return;
		}
		...
		LoginServer.getInstance().sendBanPacket(type, accountId, "", time,
				admin.getObjectId());
	}

	public static void main(String[] args) {
		AdminCommandChatHandler.getInstance().registerAdminCommand(new Ban());
	}
}

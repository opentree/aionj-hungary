...
public class Info extends AdminCommand {
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if (target == null || target.equals(admin)) {
			PacketSendUtility.sendMessage(admin,
					"Your object id is : " + admin.getObjectId());
		} else {
			if (target instanceof Npc) {
				...
				PacketSendUtility.sendMessage(admin, "[Info about npc]\n"
						...
						+ admin.getTarget().getY() + " / Z: "
						+ admin.getTarget().getZ() + " / Heading: "
	...
	public static void main(String[] args) {
		AdminCommandChatHandler.getInstance().registerAdminCommand(new Info());
	}
}

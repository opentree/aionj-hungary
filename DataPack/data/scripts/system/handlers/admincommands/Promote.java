...
public class Promote extends AdminCommand {
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if (params.length != 3) {
			PacketSendUtility
					.sendMessage(admin,
							"syntax //promote <characterName> <acceslevel | membership> <mask> ");
			return;
		}
		...
		if (params[1].toLowerCase().equals("acceslevel")) {
			...
			if (mask > 1 || mask < 0) {
				PacketSendUtility
						.sendMessage(admin, "membership can be 0 or 1");
				return;
			}
		} else {
			PacketSendUtility
		...
		LoginServer.getInstance().sendLsControlPacket(admin.getAcountName(),
				player.getName(), admin.getName(), mask, type);
	}

...
public class Set extends AdminCommand {
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if (params == null || params.length < 2) {
			PacketSendUtility.sendMessage(admin,
					"syntax //set <class|exp|ap|level|title>");
			return;
		}
		...
		if (target == null) {
			PacketSendUtility.sendMessage(admin,
					"You should select a target first!");
			return;
		}

		if (params[1] == null) {
			PacketSendUtility.sendMessage(admin,
					"You should enter second params!");
			return;
		}
		...
		if (params[0].equals("class")) {
			...
			try {
				...
				PacketSendUtility.sendMessage(admin,
						"You should enter valid second params!");
				return;
			}
			...
			if (target instanceof Player) {
				...
				PacketSendUtility.sendMessage(admin, "Set your exp to "
						+ paramValue);
			}
			...
			if (target instanceof Player) {
				...
				if (player == admin) {
					PacketSendUtility.sendMessage(admin,
							"Set your Abyss Points to " + ap + ".");
				} else {
					PacketSendUtility.sendMessage(admin,
							"Set " + player.getName() + " Abyss Points to "
									+ ap + ".");
					PacketSendUtility.sendMessage(player,
							"Admin set your Abyss Points to " + ap + ".");
				}
			} else {
				PacketSendUtility.sendMessage(admin,
						"You must select a Player to set their AP.");
			}
			...
			if (target instanceof Player) {
				...
				PacketSendUtility.sendMessage(admin, "Set "
						+ player.getCommonData().getName() + " level to "
						+ level);
			}
			...
			if (target instanceof Player) {
				...
				PacketSendUtility.sendMessage(admin, "Set "
						+ player.getCommonData().getName() + " title to "
						+ titleId);
			}
		}
	}
	...
	private void setTitle(Player player, int value) {
		...
		PacketSendUtility.broadcastPacket(player, (new SM_TITLE_UPDATE(player,
				value)));
	...
	private void setClass(Player player, PlayerClass oldClass, byte value) {
		...
		if (level < 9) {
			PacketSendUtility.sendMessage(player,
					"You can only switch class after reach level 9");
			return;
		}
		if (Arrays.asList(1, 2, 4, 5, 7, 8, 10, 11)
				.contains(oldClass.ordinal())) {
		...
		default:
			PacketSendUtility
					.sendMessage(player, "Invalid class switch chosen");
			return;
		}
		player.getCommonData().setPlayerClass(playerClass);
		player.getCommonData().upgradePlayer();
		PacketSendUtility.sendMessage(player,
				"You have successfuly switched class");
	}

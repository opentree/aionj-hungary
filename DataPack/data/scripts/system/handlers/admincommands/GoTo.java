...
public class GoTo extends AdminCommand {
	...
	public void executeCommand(Player admin, String[] params) {
		...
		if (params[0].toLowerCase().equals("poeta")) {
			...
			TeleportService.teleportTo(admin,
					WorldMapType.PANDAEMONIUM.getId(), 1679, 1400, 195, 0);
			...
			PacketSendUtility
					.sendMessage(admin, "Teleported to Core Fortress.");
		}
			...
			PacketSendUtility.sendMessage(admin,
					"Teleported to Test Giant Monster.");
		}
			...
			TeleportService.teleportTo(admin, 220070000, 1868, 2746, 531, 0);
		...
		} else if (params[0].toLowerCase().equals("debrisabyss")) {
			TeleportService.teleportTo(admin, 300220000, 720, 165, 449, 0);
			PacketSendUtility.sendMessage(admin,
					"Teleported to debris of Abyss.");
		} else if (params[0].toLowerCase().equals("defilersilentera")) {
			TeleportService.teleportTo(admin, 600010000, 525, 766, 299, 0);
			PacketSendUtility.sendMessage(admin,
					"Teleported to defiler de silentera.");
		} else if (params[0].toLowerCase().equals("brushmundir")) {
			TeleportService.teleportTo(admin, 300170000, 1218, 979, 277, 0);
			PacketSendUtility.sendMessage(admin,
					"Teleported to temple de brushmundir .");
		} else if (params[0].toLowerCase().equals("novice")) {
			TeleportService.teleportTo(admin, 300200000, 180, 19, 144, 0);
			PacketSendUtility.sendMessage(admin, "Teleported to novice.");
		} else if (params[0].toLowerCase().equals("cromede")) {
			TeleportService.teleportTo(admin, 300230000, 482, 640, 201, 0);
			PacketSendUtility.sendMessage(admin, "Teleported to cromede.");
		} else if (params[0].toLowerCase().equals("templesuperieur")) {
			TeleportService.teleportTo(admin, 300150000, 636, 657, 133, 0);
			PacketSendUtility.sendMessage(admin,
					"Teleported to temple superieur.");
		} else if (params[0].toLowerCase().equals("templeinferieur")) {
			TeleportService.teleportTo(admin, 300160000, 1145, 305, 110, 0);
			PacketSendUtility.sendMessage(admin,
					"Teleported to temple inferieur.");
		} else
			PacketSendUtility.sendMessage(admin,
	...
	public static void main(String[] args) {
		AdminCommandChatHandler.getInstance().registerAdminCommand(new GoTo());
	}
}

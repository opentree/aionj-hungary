...
public class _1929ASliverofDarkness extends QuestHandler {
	...
	public boolean onDialogEvent(QuestEnv env) {
		...
		if (qs.getStatus() == QuestStatus.START) {
				...
				case 10002:
					if (var == 2) {
						...
						WorldMapInstance newInstance = InstanceService
								.getNextAvailableInstance(310070000);
						InstanceService.registerPlayerWithInstance(newInstance,
								player);
						TeleportService.teleportTo(player, 310070000,
								newInstance.getInstanceId(), 338, 101, 1191, 0);
						return true;
					}
					break;
				case 10006:
			...
			case 205110:
				...
				break;
			case 700419: {
				if (qs.getQuestVars().getQuestVars() == 94
						&& env.getDialogId() == -1) {
				...
				case -1:
					if (var == 98) {
						...
						if (player.getEquipment()
								.getEquippedItemsByItemId(itemId).size() != 0) {
							qs.setQuestVar(96);
							updateQuestStatus(player, qs);
						}
						return false;
					...
					if (var == 96) {
						...
						ThreadPoolManager.getInstance().schedule(
								new Runnable() {
									@Override
									public void run() {
										QuestService.addNewSpawn(310070000,
												instanceId, 212992,
												(float) 191.9, (float) 267.68,
												(float) 1374, (byte) 0, true);
										qs.setQuestVar(97);
										updateQuestStatus(player, qs);
									}
								}, 5000);
	...
	public boolean onMovieEndEvent(QuestEnv env, int movieId) {
		...
		if (qs == null || qs.getStatus() != QuestStatus.START
				|| qs.getQuestVars().getQuestVars() != 94)
			return false;
		QuestService.addNewSpawn(310070000, instanceId, 205111, (float) 197.6,
				(float) 265.9, (float) 1374.0, (byte) 0, true);
		qs.setQuestVar(98);
		updateQuestStatus(player, qs);
		return true;
	}
	...
	public boolean onKillEvent(QuestEnv env) {
		...
		if (targetId == 212992 && qs.getQuestVars().getQuestVars() == 97) {
			...
			ThreadPoolManager.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					TeleportService.teleportTo(player, 210030000, 1, 2315.9f,
							1800f, 195.2f, 0);
				}
			}, 5000);
			return true;
	...
	private void removeStigma(Player player) {
		int itemId = getStoneId(player);
		List<Item> items = player.getEquipment().getEquippedItemsByItemId(
				itemId);

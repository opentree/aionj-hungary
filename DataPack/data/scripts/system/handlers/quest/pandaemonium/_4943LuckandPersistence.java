...
public class _4943LuckandPersistence extends QuestHandler {
	...
	private static final Logger log = Logger
			.getLogger(_4943LuckandPersistence.class);
				...
				if (var == 1) {
					...
					case 1354:
						...
						if (ItemService.decreaseKinah(player, 3400000)) {
							...
							return sendQuestDialog(player, env
									.getVisibleObject().getObjectId(), 1438);
						}
					}
				}
				break;
			// 4 - Better purify yourself! Take Glossy Holy Water and visit High
			// Priest Balder
			case 204075:
				if (var == 3) {
					switch (env.getDialogId()) {
					...
					case 25:
						if (player.getInventory().getItemCountByItemId(
							...
							return sendQuestDialog(player, env
									.getVisibleObject().getObjectId(), 2034);
						else
							// Send select5_2 to eddit-HtmlPages.xml
							return sendQuestDialog(player, env
									.getVisibleObject().getObjectId(), 2120);

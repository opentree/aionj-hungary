...
public class _1075NewWings extends QuestHandler {
	...
	private final static int[] npc_ids = { 278506, 279023, 278643 };// Remove
																	// mobs out
																	// of
																	// reshanta
																	// xml 4x
		...
		if (targetId == 278506) {
			...
			case 10001:
				if (var == 1) {
					...
					PacketSendUtility.sendPacket(player, new SM_EMOTION(player,
							EmotionType.START_FLYTELEPORT, 57001, 0));
					return true;
				}
			}
			...
			case 10002:
				if (var == 2) {
					qs.setQuestVarById(0, var + 1);
					updateQuestStatus(player, qs);
					QuestService.addNewSpawn(400010000, 1, 214102, 2344.32f,
							1789.96f, 2258.88f, (byte) 86, true);
					QuestService.addNewSpawn(400010000, 1, 214102, 2344.51f,
							1786.01f, 2258.88f, (byte) 52, true);
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
							env.getVisibleObject().getObjectId(), 0));
					return true;
				}

...
public class _1002RequestoftheElim extends QuestHandler {
	...
	public boolean onDialogEvent(QuestEnv env) {
		...
		if (targetId == 203076) {
				...
				if (var == 1) {
					if (player.getInventory().getItemCountByItemId(182200002) == 0)
						if (!ItemService.addItems(player, Collections
								.singletonList(new QuestItems(182200002, 1))))
							return true;
					qs.setQuestVarById(0, var + 1);
					updateQuestStatus(player, qs);
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
			...
			switch (env.getDialogId()) {
			case -1:
				List<Item> items = player.getInventory().getItemsByItemId(
						182200002);
				if (items.isEmpty())
					return false;
				final int targetObjectId = env.getVisibleObject().getObjectId();
				PacketSendUtility.sendPacket(player,
						new SM_USE_OBJECT(player.getObjectId(), targetObjectId,
								3000, 1));
				PacketSendUtility.broadcastPacket(player,
						new SM_EMOTION(player, EmotionType.START_QUESTLOOT, 0,
								targetObjectId), true);
				ThreadPoolManager.getInstance().schedule(new Runnable() {
					...
					public void run() {
						...
						PacketSendUtility.sendPacket(player, new SM_USE_OBJECT(
								player.getObjectId(), targetObjectId, 3000, 0));
						PacketSendUtility.broadcastPacket(player,
								new SM_EMOTION(player,
										EmotionType.END_QUESTLOOT, 0,
										targetObjectId), true);
						PacketSendUtility.sendPacket(player,
								new SM_DIALOG_WINDOW(targetObjectId, 10));
					}
				}, 3000);
				return true;
			case 25:
				...
				((Npc) env.getVisibleObject()).onDie(null); // TODO check null
															// or player
				return true;
			default:
				return false;
				...
				if (var == 13) {
					PacketSendUtility.sendPacket(player,
							new SM_DIALOG_WINDOW(env.getVisibleObject()
									.getObjectId(), 2375, questId));
					return true;
				} else if (var == 14) {
					PacketSendUtility.sendPacket(player,
							new SM_DIALOG_WINDOW(env.getVisibleObject()
									.getObjectId(), 2461, questId));
					return true;
				}
				return false;
			case 10004:
			...
			case 25:
				if (var == 20) {
					PacketSendUtility.sendPacket(player, new SM_EMOTION(player,
					...
					ThreadPoolManager.getInstance().schedule(new Runnable() {
						...
						public void run() {
							...
							updateQuestStatus(player, qs);
							TeleportService.teleportTo(player, 210010000, 1,
									603, 1537, 116, (byte) 20, 0);
						}
					}, 43000);
					return true;
				}

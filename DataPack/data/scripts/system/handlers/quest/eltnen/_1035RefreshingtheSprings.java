...
public class _1035RefreshingtheSprings extends QuestHandler {
	...
	private final static int[] npc_ids = { 203917, 203992, 700158, 203965,
			203968, 203987, 700160, 203934, 700159 };
			...
			case 10005:
				if (var == 6) {
					...
					ItemService.addItems(player, Collections
							.singletonList(new QuestItems(182201024, 1)));
					return true;
				}
			case 10006:
				if (var == 8) {
					...
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
							env.getVisibleObject().getObjectId(), 10));
					ItemService.addItems(player, Collections
							.singletonList(new QuestItems(182201025, 1)));
					return true;
				}
				return false;
			}

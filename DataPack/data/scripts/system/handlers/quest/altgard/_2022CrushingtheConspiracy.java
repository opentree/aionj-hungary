...
public class _2022CrushingtheConspiracy extends QuestHandler {
		...
		if (qs == null) {
			...
			case 700140: {
				if (qs.getQuestVarById(0) == 1) {
					qs.setQuestVarById(0, qs.getQuestVarById(0) + 1);
					updateQuestStatus(player, qs);
					TeleportService.teleportTo(player, 320030000, 275.68f,
							164.03f, 205.19f, 34);
						...
						public void run() {
							...
							PacketSendUtility.broadcastPacket(player,
									new SM_EMOTION(player,
											EmotionType.START_LOOT, 0, 700142),
									true);
							QuestService.addNewSpawn(320030000, 1, 210753,
									(float) 260.12, (float) 234.93,
									(float) 216.00, (byte) 90, true);
						}
					}, 3000);
					return true;
				}
			...
			case 700141: {
				if (qs.getQuestVarById(0) == 4) {
					...
					updateQuestStatus(player, qs);
					TeleportService.teleportTo(player, 220030000, 2453.0f,
							2553.2f, 316.3f, 26);
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
							env.getVisibleObject().getObjectId(), 10));
					return true;
	...
	public boolean onLvlUpEvent(QuestEnv env) {
		...
		if (qs == null || qs.getStatus() != QuestStatus.LOCKED)
			return false;
		int[] quests = { 2200, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018,
				2019, 2020, 2021 };
		for (int id : quests) {

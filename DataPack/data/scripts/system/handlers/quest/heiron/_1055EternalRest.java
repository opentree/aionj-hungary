...
public class _1055EternalRest extends QuestHandler {
	...
	private final static int[] npc_ids = { 204629, 204625, 204628, 204627,
			204626, 204622, 700270 };
			...
			case 33:
				if (QuestService.collectItemCheck(env, true)) {
					...
					ItemService.addItems(player, Collections
							.singletonList(new QuestItems(182201613, 1)));
					return sendQuestDialog(player, env.getVisibleObject()
							.getObjectId(), 10000);
				} else
					return sendQuestDialog(player, env.getVisibleObject()
			...
			case 10002:
				if (var == 2) {
					if (player.getInventory().getItemCountByItemId(182201609) == 0)
						ItemService.addItems(player, Collections
								.singletonList(new QuestItems(182201609, 1)));
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
							env.getVisibleObject().getObjectId(), 10));
					return true;
				}
			...
			case 10002:
				if (var == 2) {
					if (player.getInventory().getItemCountByItemId(182201610) == 0)
						ItemService.addItems(player, Collections
								.singletonList(new QuestItems(182201610, 1)));
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
							env.getVisibleObject().getObjectId(), 10));
					return true;
				}
			...
			case 10002:
				if (var == 2) {
					if (player.getInventory().getItemCountByItemId(182201611) == 0)
						ItemService.addItems(player, Collections
								.singletonList(new QuestItems(182201611, 1)));
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
							env.getVisibleObject().getObjectId(), 10));
					return true;
				}
			...
			case 10002:
				if (var == 2) {
					if (player.getInventory().getItemCountByItemId(182201612) == 0)
						ItemService.addItems(player, Collections
								.singletonList(new QuestItems(182201612, 1)));
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
							env.getVisibleObject().getObjectId(), 10));
					return true;
				}

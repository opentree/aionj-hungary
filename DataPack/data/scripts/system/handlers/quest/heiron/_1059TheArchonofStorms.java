...
public class _1059TheArchonofStorms extends QuestHandler {
			...
			case 10004:
				if (var == 4) {
					...
					ItemService.addItems(player, Collections
							.singletonList(new QuestItems(182201619, 1)));
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(
							env.getVisibleObject().getObjectId(), 10));
					return true;
				}
	...
	public boolean onMovieEndEvent(QuestEnv env, int movieId) {
		...
		if (qs == null || qs.getStatus() != QuestStatus.START
				|| qs.getQuestVars().getQuestVars() != 2)
			return false;
		qs.setQuestVar(3);
		updateQuestStatus(player, qs);
		player.getEffectController().setAbnormal(
				EffectId.SHAPECHANGE.getEffectId());
		player.setTransformedModelId(212000);
		PacketSendUtility.broadcastPacketAndReceive(player, new SM_TRANSFORM(
				player));
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			@Override
			public void run() {

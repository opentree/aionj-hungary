...
public class MySQL5LegionDAO extends LegionDAO
{
	...
	public Legion loadLegion(final String legionName)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				legion.setLegionPermissions(rset.getInt("legionar_permission2"), rset.getInt("centurion_permission1"), rset.getInt("centurion_permission2"));
	...
	public LegionEmblem loadLegionEmblem(final int legionId)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				legionEmblem.setEmblem(rset.getInt("emblem_id"), rset.getInt("color_r"), rset.getInt("color_g"), rset.getInt("color_b"));
			}
	...
	public LegionWarehouse loadLegionStorage(Legion legion)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				Item item = new Item(legionId, itemUniqueId, itemId, itemCount, itemColor, isEquiped == 1, false, slot, storage, enchant, itemSkin,
						fusionedItem);
	...
	public void loadLegionHistory(final Legion legion)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				history.add(new LegionHistory(LegionHistoryType.valueOf(rset.getString("history_type")), rset.getString("name"), rset.getTimestamp("date")));
			}

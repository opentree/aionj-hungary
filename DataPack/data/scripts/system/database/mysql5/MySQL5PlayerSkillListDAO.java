...
public class MySQL5PlayerSkillListDAO extends PlayerSkillListDAO {
	private static final Logger log = Logger
			.getLogger(MySQL5PlayerSkillListDAO.class);
	...
	public SkillList loadSkillList(final int playerId) {
		...
		try {
			...
			while (rset.next()) {
				...
				skills.put(id, new SkillListEntry(id, false, lv,
						PersistentState.UPDATED));
			}
			rset.close();
			stmt.close();
		} catch (Exception e) {
			log.fatal("Could not restore SkillList data for player: "
	...
	public boolean storeSkills(Player player) {
		...
		SkillListEntry[] skillsDeleted = player.getSkillList()
				.getDeletedSkills();
	...
	private void store(Player player, SkillListEntry[] skills) {
		for (int i = 0; i < skills.length; i++) {
			...
			case NEW:
				addSkill(player.getObjectId(), skill.getSkillId(),
						skill.getSkillLevel());
				break;
			case UPDATE_REQUIRED:
				updateSkill(player.getObjectId(), skill.getSkillId(),
						skill.getSkillLevel());
				break;
			case DELETED:
	 ...
	 *            skill contents.
	 */
	private void addSkill(final int playerId, final int skillId,
	 ...
	 * @param skillLevel
	 */
	private void updateSkill(final int playerId, final int skillId,
			final int skillLevel) {

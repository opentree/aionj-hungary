...
public class MySQL5PetitionDAO extends PetitionDAO
{
	...
	public Petition getPetitionById(int petitionId)
	{
		...
		try
		{
			...
			result = new Petition(rset.getInt("id"), rset.getInt("playerId"), rset.getInt("type"), rset.getString("title"), rset.getString("message"),
	...
	public Set<Petition> getPetitions()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				Petition p = new Petition(rset.getInt("id"), rset.getInt("playerId"), rset.getInt("type"), rset.getString("title"), rset.getString("message"),
						rset.getString("addData"), status.getElementId());
				results.add(p);
			}
			stmt.close();

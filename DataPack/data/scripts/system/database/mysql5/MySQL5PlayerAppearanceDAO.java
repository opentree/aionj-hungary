...
/**
 * Class that is responsible for loading/storing
 * {@link com.aionemu.gameserver.model.gameobjects.player.PlayerAppearance} in
 * mysql5
 * 
...
public class MySQL5PlayerAppearanceDAO extends PlayerAppearanceDAO {
	private static final Logger log = Logger
			.getLogger(PlayerAppearanceDAO.class);
	...
	public PlayerAppearance load(final int playerId) {
		...
		try {
			...
			PreparedStatement statement = con
					.prepareStatement("SELECT * FROM player_appearance WHERE player_id = ?");
			...
			log.fatal("Could not restore PlayerAppearance data for player "
				...
				.insertUpdate(
						"REPLACE INTO player_appearance ("
							...
							public void handleInsertUpdate(PreparedStatement ps)
									throws SQLException {
								log.debug("[DAO: MySQL5PlayerAppearanceDAO] storing appereance "
										+ id);
								ps.setInt(1, id);

...
public class Equipment
{
	...
	public Item equipItem(int itemUniqueId, int slot)
	{
		...
		if (itemTemplate.getLevel() > owner.getCommonData().getLevel())
		{
			PacketSendUtility.sendPacket(
					owner,
					SM_SYSTEM_MESSAGE.STR_CANNOT_USE_ITEM_TOO_LOW_LEVEL_MUST_BE_THIS_LEVEL(itemTemplate.getLevel(),
							new DescriptionId(Integer.parseInt(item.getName()))));
			return null;
		}
	...
	private boolean soulBindItem(final Player player, final Item item, final int slot)
	{
		RequestResponseHandler responseHandler = new RequestResponseHandler(player)
		{
			...
			public void acceptRequest(StaticNpc requester, Player responder)
			{
				...
				ThreadPoolManager.getInstance().schedule(new Runnable()
				{
					...
					public void run()
					{
						...
						PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.SOUL_BOUND_ITEM_SUCCEED(new DescriptionId(item.getNameID())));
			...
			{
				PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.SOUL_BOUND_ITEM_CANCELED(new DescriptionId(item.getNameID())));
			}
		};
		...
		if (requested)
		{
			PacketSendUtility.sendPacket(player, new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_SOUL_BOUND_ITEM_DO_YOU_WANT_SOUL_BOUND, 0, new DescriptionId(
					item.getNameID())));
		}

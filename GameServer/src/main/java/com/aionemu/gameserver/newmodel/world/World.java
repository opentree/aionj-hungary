...
import com.aionemu.gameserver.newmodel.gameobject.AionObject;
...
public class World
{
	 ...
	 * @param newHeading
	 */
	public void updatePosition(SpawnedObject object, float newX, float newY, float newZ, byte newHeading)
	{
	 ...
	 * @param newHeading
	 */
	public void updatePosition(SpawnedObject object, float newX, float newY, float newZ, byte newHeading, boolean updateKnownList)
	{
		...
		if(oldRegion == null)
		{
			log.warn(String.format("CHECKPOINT: oldregion is null, object coordinates - %d %d %d", object.getPosition().getX(), object.getPosition().getY(), object.getPosition().getZ()));
			return;
		}
	 ...
	 */
	public void setPosition(SpawnedObject object, int mapId, float x, float y, float z, byte heading)
	{
		int instanceId = 1;	
		if(object.getPosition().getMapId() == mapId)
		{
			instanceId = object.getPosition().getInstanceId();
		}
	 ...
	 * @param heading
	 */
	public void setPosition(SpawnedObject object, int mapId, int instance, float x, float y, float z, byte heading)
	{
		if(object.getPosition().isSpawned())
			despawn(object);
	 ...
	 */
	public void spawn(SpawnedObject object)
	{
		if(object.getPosition().isSpawned())
		...
		if(object.getSpawn() != null)
			object.getSpawn().setSpawned(true, object.getPosition().getInstanceId());
	 ...
	 */
	public void despawn(SpawnedObject object)
	{
		...
		if(object.getSpawn() != null)
			object.getSpawn().setSpawned(false, object.getPosition().getInstanceId());

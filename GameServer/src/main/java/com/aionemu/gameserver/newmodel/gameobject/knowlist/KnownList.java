...
public class KnownList
{
	...
	private final SpawnedObject					owner;

	private final Map<Integer, SpawnedObject>	knownObjects		= new SingletonMap<Integer, SpawnedObject>().setShared();
	 ...
	 * @param owner
	 */
	public KnownList(SpawnedObject owner)
	{
		this.owner = owner;
	}
	 ...
	 */
	public final Map<Integer, SpawnedObject> getKnownObjects()
	{
		return knownObjects;
	}
	...
	{
		for(SpawnedObject object : getKnownObjects().values())
		{
	 ...
	 * @return
	 */
	protected boolean addKnownObject(SpawnedObject object)
	{
	 ...
	 * @return
	 */
	private final boolean removeKnownObject(SpawnedObject object, boolean isOutOfRange)
	{
		if(object == null)
			return false;
	...
	private final boolean forgetVisibleObject(SpawnedObject object)
	{
		if(checkObjectInRange(getOwner(), object))
			return false;
	...
	protected final boolean checkObjectInRange(SpawnedObject owner, SpawnedObject newObject)
	{
		// check if Z distance is greater than maxZvisibleDistance
		if(Math.abs(owner.getPosition().getZ() - newObject.getPosition().getZ()) > maxZvisibleDistance)
			return false;

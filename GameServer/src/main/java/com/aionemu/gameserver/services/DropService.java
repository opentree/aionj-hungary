...
public class DropService
{
	...
	public void registerDrop(StaticNpc npc, Player player, int lvl)
	{
		...
		{
			dropRegistrationMap.put(npcUniqueId, new DropNpc(GroupService.getInstance().getMembersToRegistrateByRules(player, player.getPlayerGroup(), npc)));
					...
					if (dropNpc.getDistributionType() > 1 && !dropNpc.isInUse())
					{
						...
						for (PlayerAllianceMember allianceMember : player.getPlayerAlliance().getMembers())
						{
							...
							if (member.isOnline())
							{
								dropNpc.addPlayerStatus(member);
								PacketSendUtility.sendPacket(member,
										new SM_GROUP_LOOT(member.getPlayerAlliance().getPlayerAllianceGroupForMember(member.getObjectId()).getAllianceId(),
	...
	private void winningRollActions(Player player, int itemId, int npcId)
	{
		...
		if (player.isInGroup())
		{
			...
			{
				if (!player.equals(member))
					PacketSendUtility.sendPacket(member, SM_SYSTEM_MESSAGE.STR_MSG_LOOT_GET_ITEM_OTHER(player.getName(), new DescriptionId(nameId)));
			}
		}
		if (player.isInAlliance())
		{

...
public class PlayerController extends CreatureController<Player>
{
	...
	public void onEnterZone(ZoneInstance zoneInstance)
	{
		...
		WorldType worldType = World.getInstance().getWorldMap(getOwner().getWorldId()).getWorldType();
		if(worldType == WorldType.ABYSS)
			checkEnemyAbyssBase(getOwner());
	}
	 ...
	 *
	 */
	public void checkEnemyAbyssBase(final Player player)
	{
		if(player.getLifeStats().isAlreadyDead() || player.isGM())
			return;
		if ( (player.getCommonData().getRace() == Race.ELYOS) && (MathUtil.isInSphere(player,878.6906f,2950.4717f,1646.4772f,290)) )
			die();
		else if ( (player.getCommonData().getRace() == Race.ASMODIANS) && (MathUtil.isInSphere(player,2979.622f,906.5588f,1540.2731f,290)) )
			die();
	}

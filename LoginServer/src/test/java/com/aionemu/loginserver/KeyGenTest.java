...
public class KeyGenTest {
	...
	public void testRSAPreInit() throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			RSAKeyPairs[i] = new EncryptedRSAKeyPair(
		...
		for (int i = 0; i < 10; i++) {
			...
			pRsaCipher.init(Cipher.DECRYPT_MODE, RSAKeyPairs[i].getRSAKeyPair()
					.getPrivate());
		}
		...
		for (int i = 0; i < 10; i++) {
			...
			rsaCipher.init(Cipher.DECRYPT_MODE, RSAKeyPairs[0].getRSAKeyPair()
					.getPrivate());
